...
public class ModifiableStat
{
    ...
    List<StatChanger> _changes = new();
    public void AddChanger(StatChanger change)
    {
        _changes.Add(change);
        RecalculateValue();
    }
    public void RemoveChanger(StatChanger change)
    {
    ...
    void RecalculateValue()
    {
        ...
        foreach (var modifier in _changes)
        {
            CalculateBaseStatChange(modifier, StartingValue);
        }
        float newInitialValue = CurrentValue;
        foreach (var modifier in _changes)
        {
            CalculateSecondStatChange(modifier, newInitialValue);
        }
        void CalculateBaseStatChange(StatChanger statChange, float InitialValue)
        {

            CurrentValue += (statChange.BasePercentChange / 100f) * InitialValue;
            CurrentValue += statChange.BaseAdditionChange;

        }
        void CalculateSecondStatChange(StatChanger statChange, float InitialValue)
        {
            CurrentValue += (statChange.PerecentChange / 100f) * InitialValue;
            CurrentValue += statChange.AddtionChange;
        }
    }
}
