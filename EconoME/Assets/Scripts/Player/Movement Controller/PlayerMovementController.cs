...
public class PlayerMovementController : MonoBehaviour
{
    ...
    [field: SerializeReference] public ModifiableFloatStat PlayerSpeed { get; private set; } = new ModifiableFloatStat();
    ...
    public bool CanMove
    {
        get
        {
            return !UIOpen.TrueValueExists()
                && !NewPlayerAnimationController.Instance.AnimationLocked
                && !PlayerCombatController.Instance.Stunned
                && !WorldTimeManager.Instance.TimePaused;
        }
    }
    ...
    public void Awake()
    {
        ...
        //Player Input
        _moveAction = CustomInputManager.Instance.MoveKeys;
        _sprintAction = CustomInputManager.Instance.LeftShift;

    }

    private void Start()
    {
        PlayerPosition.Value = Rigidbody.transform.position;
    ...
    void FixedUpdate()
    {
        ...
        Rigidbody.MovePosition(Rigidbody.position + movements);
        PlayerPosition.Value = Rigidbody.transform.position;
    }

    internal void UpdatePlayerPos()
    {
