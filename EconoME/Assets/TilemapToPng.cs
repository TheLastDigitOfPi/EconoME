    ...
    public class TilemapToPng : MonoBehaviour
{
        ...
        for (int x = minX; x <= maxX; x++)
        {
            for(int y = minY; y <= maxY; y++)
            {
                ...
                { 
                    ...
                    var nextSprite = GetCurrentSprite(tm.GetSprite(new Vector3Int(x,y,0)));
                    Imagen.SetPixels32((x - minX) * 32, (y - minY) * 32,nextSprite.width, nextSprite.height,nextSprite.GetPixels32());
                    if(nextSprite.width != 32 || nextSprite.height != 32)
                    {
                        Debug.Log("Sprite size was " + nextSprite.width + " by " + nextSprite.height + " pixels named: " + tm.GetSprite(new Vector3Int(x,y,0)).name);
                    }
                    
                    //Imagen.SetPixels((x - minX) * (int)width, (y - minY) * (int)height, (int)width, (int)height, nextSprite);
                }
            }
        }
        Imagen.Apply();
    ...
    {
        var pixels = sprite.texture.GetPixels((int)((int)sprite.textureRect.x - (32-sprite.textureRect.width)),
                                         (int)((int)sprite.textureRect.y - (32-sprite.textureRect.height)),
                                         32,
                                         32);
